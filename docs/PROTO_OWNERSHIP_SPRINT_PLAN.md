# NodeForge Proto Ownership Sprint Plan
...
이 문서는 `NodeForge` 가 자기 proto source ownership을 회수하고, 외부 `api-protos` 경로와 workspace 결합을 제거하기 위한 스프린트 일정을 고정한다.

현재 기준선:

- `nodeforge.proto` 는 이미 `NodeForge` 서비스 구현과 in-cluster gRPC/GRPCRoute 노출에 직접 연결돼 있다.
- `NodeForge` 빌드 체인은 `api-protos/gen/go/nodeforge/v1` 경로에 강하게 묶여 있다.
- proto ownership 정리와 build/vendor/workspace 정리는 한 번에 끝내지 않고 스프린트로 나눈다.

## Ownership Scope

이번 계획에서 `NodeForge` 가 회수할 대상:
...
`NodeForge` 쪽 proto ownership 정리 작업이 진행되는 동안 아래 범위는 임의 변경을 피한다.
...
- `NodeForge` 의 proto import path
- `NodeForge/go.work`, Dockerfile, vendor 절차 중 proto 경로 관련 부분
...
- `NodeForge` 가 회수할 proto 범위가 문서상 분명하다
...
- canonical source 초안이 `NodeForge/protos/` 아래에 추가되었다
- active import path 는 아직 external generated path 를 유지한다
- `go.work`, Dockerfile, vendor 절차는 아직 외부 `api-protos` 경로를 본다

할 일:

- `nodeforge.proto` 를 `NodeForge` 저장소 안 canonical 위치로 이동
- `tool_service.proto` 를 `NodeForge` 버킷으로 이동
- `volres_service.proto` 를 `NodeForge` 버킷으로 이동
- 새 canonical source 기준으로 code generation entry point 를 정의

완료 기준:

- `NodeForge` 저장소 안에 source `.proto` canonical 위치가 생긴다
- 회수 대상 3개 proto 가 더 이상 `api-protos` canonical source 로 읽히지 않는다
- generation 흐름을 `NodeForge` 내부에서 시작할 수 있다

Sprint 1 canonical paths:

- `NodeForge/protos/nodeforge/v1/nodeforge.proto`
- `NodeForge/protos/tool/ichthys/v1/tool_service.proto`
- `NodeForge/protos/volres/ichthys/v1/volres_service.proto`
...
- local source 기준 `nodeforge` generated code 가 `NodeForge/protos/nodeforge/v1` 아래에 생성되었다
...
- `NodeForge` 코드 import 를 새 local generated path 로 전환
- test/build 에서 새 generated path 사용 확인
- `tool`, `volres` 가 아직 구현 연결이 약한 경우에도 source ownership 기준만 먼저 고정

완료 기준:

- active `NodeForge` 코드가 `api-protos/gen/go/nodeforge/v1` 를 요구하지 않는다
- source `.proto` 와 generated path 의 owner 가 모두 `NodeForge` 가 된다
...
- `NodeForge` 는 `api-protos` 제거 시 추가 조치 없이 자기 proto source 와 generated path 로 동작한다

할 일:

- `NodeForge` 의 active build/test/deploy 경로에 `api-protos` 의존이 없는지 최종 확인
- deployment 문서와 proto ownership 문서를 새 기준으로 정리
- `api-protos` sunset plan exit criteria 중 `NodeForge` 책임 항목 완료 확인

완료 기준:

- `NodeForge` 는 자기 source `.proto` 를 직접 소유한다
- `NodeForge` build/test/deploy 경로가 `api-protos` 를 요구하지 않는다
- `api-protos` 제거 시 `NodeForge` 쪽 blocker 가 없다

최종 상태:

- `NodeForge/protos/` 가 canonical source 다
- `NodeForge/protos/nodeforge/v1` generated path 가 active import 경로다
- `go.work` 없이 `go mod` 와 `vendor` 만으로 빌드/테스트가 통과한다
- `api-protos` 는 문서상 sunset 대상일 뿐, `NodeForge` 의 active dependency 가 아니다

## Recommended Order Inside NodeForge
...
- 현재 다른 작업자가 `NodeForge` proto/build 경로를 동시에 수정하면 migration 충돌이 날 수 있다
...
- `NodeForge` runtime 기능 확장보다 proto ownership 회수를 우선
