...
[1] 빌드 레시피        Dockerfile + conda spec (사용자 작성)
         ↓ L2 빌드 (podbridge5)
[2] 배포 인프라 YAML   NodeForge K8s Deployment/ConfigMap/Service
         ↓ gRPC BuildRequest
[3] ToolDefinition     NodeKit 작성 → NodeForge 수신 → CAS JSON 저장
         ↓ TODO-07 (미구현)
...
이 내용이 `BuildRequest.dockerfile_content` 필드로 NodeForge에 전송된다.
...
NodeForge가 받아서 podbridge5 in-process 빌드로 전달한다.
...
## 계층 2 — 배포 인프라 YAML (NodeForge K8s 리소스)

### 역할

NodeForge 바이너리가 K8s 위에서 실행되기 위한 Deployment / ConfigMap / Service / GRPCRoute.
...
- NodeForge: `nodeforge` namespace, 1/1 Running (2026-04-18 기준)
...
최종 목표는 `nodevault`로 rename이지만, **api-protos freeze가 해제될 때까지 이름 변경 불가**.
(이유: proto import path, go.work, Dockerfile 모두 api-protos와 묶여 있음)
api-protos cleanup (PROTO_OWNERSHIP_SPRINT_PLAN.md Sprint 3/4) 완료 후 rename 진행.

---

## 계층 3 — ToolDefinition / CAS JSON (NodeKit ↔ NodeForge 계약)

### 역할

NodeKit에서 관리자가 툴을 정의한 내용을 proto 직렬화해 NodeForge에 전송하고,
NodeForge가 이를 CAS 파일로 저장하는 중간 표현이다.
...
- NodeForge `pkg/catalogrest`: `GET /api/v1/tools` → `index.Store.ListActive()`
- NodeForge `pkg/index`: 이중 축 상태 관리 (lifecycle_phase + integrity_health), 15개 테스트 통과
...
| 2. 배포 인프라 YAML | NodeForge K8s Deployment | 동작 중 (이름은 NodeForge) |
| 3. ToolDefinition / CAS | NodeKit ↔ NodeForge 계약, pkg/catalog + pkg/index | 동작 중 (TODO-04 일부 미완) |
...
- **api-protos cleanup**: PROTO_OWNERSHIP_SPRINT_PLAN.md Sprint 3/4 완료 후 NodeForge → NodeVault rename
