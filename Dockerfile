# ── Stage 1: Go 빌드 ──────────────────────────────────────────────────────────
# buildah 라이브러리는 CGO를 사용하므로 CGO_ENABLED=1 + C 헤더가 필요하다.
# quay.io/buildah/stable 이미지에는 buildah 런타임 + C 헤더가 모두 포함돼 있다.
#
...
# ── Stage 2: 런타임 이미지 ────────────────────────────────────────────────────
# buildah가 Pod 내에서 이미지를 빌드하려면 overlay + runc 런타임이 필요하다.
FROM quay.io/buildah/stable:v1.37.1

COPY --from=builder /bin/nodeforge /usr/local/bin/nodeforge

# buildah 스토리지 디렉토리 (K8s Deployment에서 emptyDir 볼륨으로 마운트)
